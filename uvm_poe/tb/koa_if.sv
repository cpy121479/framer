...
interface koa_if #(parameter int NUM_OH_PLANES  = 4,
  ...
  logic [NUM_OH_PLANES*KO_W-1:0] oh_e_data;
  ...
  logic [NUM_OH_PLANES*KO_W-1:0] oh_i_data;
  ...
  logic [NUM_X2X_PLANES*KO_W-1:0] aps_e_data;
  ...
  logic [NUM_X2X_PLANES*KO_W-1:0] aps_i_data;
  ...
  logic [NUM_X2X_PLANES*KO_W-1:0] alm_data;
  ...
  logic            u_e_vld;
  logic [KO_W-1:0] u_e_data;
  logic [2:0]      u_e_pri;
  logic            u_e_rdy;
  logic            u_i_vld;
  logic [KO_W-1:0] u_i_data;
  logic [2:0]      u_i_pri;
  logic            u_i_rdy;
  // ---- KO 输出 ----
  logic            out_vld;
  logic [KO_W-1:0] out_data;
  logic [2:0]      out_pri;
  logic [2:0]      out_src;   // 优先级组号 0..7
  logic [2:0]      out_stream; // 来源流 0..6
  logic [CID_W-1:0] out_cid;
  logic [POS_W-1:0] out_pos;
  // ---- 接收确认（KOA 实际入队，给 monitor/scoreboard） ----
  logic            ack_vld;
  logic [2:0]      ack_stream;
  logic [2:0]      ack_pri;
  logic [KO_W-1:0] ack_data;
  logic [CID_W-1:0] ack_cid;
  logic [POS_W-1:0] ack_pos;

  clocking drv_cb @(posedge clk);
    default input #1step output #1;
    output oh_e_vld, oh_e_data, oh_e_pri, oh_e_cid, oh_e_pos,
           oh_i_vld, oh_i_data, oh_i_pri, oh_i_cid, oh_i_pos,
           aps_e_vld, aps_e_data, aps_e_pri, aps_e_cid, aps_e_pos,
           aps_i_vld, aps_i_data, aps_i_pri, aps_i_cid, aps_i_pos,
           alm_vld, alm_data, alm_pri, alm_cid, alm_pos,
    ...
    input  oh_e_rdy, oh_i_rdy, aps_e_rdy, aps_i_rdy, alm_rdy, u_e_rdy, u_i_rdy,
           ...
           ack_vld, ack_data, ack_pri, ack_stream, ack_cid, ack_pos;
  endclocking

  clocking mon_cb @(posedge clk);
    default input #1step;
    input oh_e_vld, oh_e_data, oh_e_pri, oh_e_rdy,
          oh_e_cid, oh_e_pos,
          oh_i_vld, oh_i_data, oh_i_pri, oh_i_rdy, oh_i_cid, oh_i_pos,
          aps_e_vld, aps_e_data, aps_e_pri, aps_e_rdy, aps_e_cid, aps_e_pos,
          aps_i_vld, aps_i_data, aps_i_pri, aps_i_rdy, aps_i_cid, aps_i_pos,
